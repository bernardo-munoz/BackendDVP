...
import { Roles, MenuRol } from '../users/model/user';
...
export class RolesComponent implements OnInit {
  ...
  delRol(){
    if (this.name_rol === "" || this.name_rol === undefined) {
      ...
      const selectedRole = this.roles.find(role => role.rol === this.name_rol);

      if(selectedRole)
        this.userService.setRol(selectedRole.id_rol , this.name_rol, "0").subscribe((data: RequestResultPHP<Roles>) => {
          if (data.success == "1") {
  ...
  setRol(){
    if (this.name_rol === "" || this.name_rol === undefined) {
      ...
      this.userService.setRol(this.selectedRol, this.name_rol, "1").subscribe((data: RequestResultPHP<Roles>) => {
        if (data.success == "1") {
          this.toastr.success(data.message); 
          ...
          // Establecer el ultimo elemento como la opción predeterminada
          if (this.selectedRol == "0" && this.roles.length > 0) {
            this.selectedRol = this.roles[this.roles.length - 1].id_rol;
          }
